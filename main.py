...
p2p = QiwiP2P(Auth.qiwi_auth, skip_key_validation=True)
Other.slash = slash
Other.p2p = p2p

for file in os.listdir('./cogs'):
    if file.endswith(
            '.py') and not file in ["config.py", "mongo.py", "messages.py"]:
...
async def on_ready():
    ...
    await bot.change_presence(
        ...
        activity=discord.Streaming(name="hp.help",
                                   url="https://twitch.tv/twitch")
    )  #https://private-cp.tk | Серверов: {word.unit(len(bot.guilds))


# пробуй
# перезапусти бота / не через кнопку run python3 main.py
@bot.event
...
async def on_guild_join(guild):
    def first(guild):
        for i in guild.text_channels:
            if i.permissions_for(guild.me).send_messages and i.permissions_for(
                    guild.me).read_messages and i.permissions_for(
                        guild.me).embed_links:
                return i
...
`2.` Убедись, что у меня есть права администратора для работы.
            ...
            row = ActionRow(
                Button(style=ButtonStyle.link,
                       label="Поддержка",
                       emoji="❔",
                       url="https://discord.gg/jyDkucBRvtp")  #,
                #Button(
            ...
            if bot.user.id == 1003367023906127982:
...
**ID:** {guild.id}
        ...
        embed.add_field(name="Причина",
                        value=cache.bl_data[guild.owner.id]["reason"])
...
async def on_guild_remove(guild):
    if bot.user.id == 740540209896095864:
        lb = discord.Embed(
            title="😢 | К сожалению, этому серверу бот не понравился")
        lb.color = Color.danger
        lb.description = f'''
**Название сервера:** {guild.name}
**Владелец:** {guild.owner}
            ...
            try:
                ...
                if 'muterole' in cache.configs_data[dictionary]:
                    muterole = guild.get_role(
                        cache.configs_data[dictionary]['muterole'])
                    for member in cache.mutes_data[dictionary]:
                        if cache.mutes_data[dictionary][member] <= int(
                                time.time()):
        ...
        for dictionary in cache.locks_data:
            try:
                ...
                for member in cache.locks_data[dictionary]:
                    if cache.locks_data[dictionary][member]['locked'] <= int(
                            time.time()):
                        user = guild.get_member(int(member))
                        for role in cache.locks_data[dictionary][member][
                                'roles']:
                            try:
                        ...
                        await mngd[0].edit(permissions=discord.Permissions(
                            permissions=cache.locks_data[dictionary][member]
                            ['managed']['perms']))
        ...
        for dictionary in cache.quarantine_data:
            try:
                ...
                for member in cache.quarantine_data[dictionary]:
                    if member.isdigit():
                        if cache.quarantine_data[dictionary][member][
                                'end'] <= int(time.time()):
                            cache.quarantine.delete(guild.id, {member: True})
                            try:
                                role = guild.get_role(
                                    cache.quarantine_data[guild.id]['role'])
                            except:
                                role = None
                            if guild.get_member(int(member)) and role:
                                await guild.get_member(int(member)
                                                       ).remove_roles(role)
            except:
                pass
        await asyncio.sleep(60)
...
async def _eval(ctx, *, code):
    ...
    if ctx.author.id in [
            356737308898099201, 685837803413962806, 750245767142441000
    ]:
        pending_embed = discord.Embed(
            title='Добрый день!',
            description='Код выполняется, подождите...',
            color=discord.Colour.from_rgb(255, 255, 0))
        message = await ctx.send(embed=pending_embed)
        success_embed = discord.Embed(title='Выполнение кода - успех',
        ...
        try:
            with contextlib.redirect_stdout(stdout):
                ...
                success_embed.add_field(name='Выполненный код:',
                                        value=f'```py\n{code}\n```',
                                        inline=False)
                what_returned = None
                if obj != None:
                    ...
                    success_embed.add_field(name='Тип данных:',
                                            value=f'```\n{what_returned}\n```',
                                            inline=False)
                    success_embed.add_field(name='Вернулось:',
                                            value=f'```\n{obj}\n```',
                                            inline=False)
                else:
                    pass
            ...
            fail_embed = discord.Embed(title='Выполнение кода - провал',
                                       color=discord.Colour.from_rgb(
                                           255, 0, 0))
            fail_embed.add_field(name='Выполненный код:',
                                 value=f'```py\n{code}\n```',
                                 inline=False)
            fail_embed.add_field(name='Ошибка:',
                                 value=f'```py\n{e}\n```',
                                 inline=False)
...
async def on_command_completion(ctx):
    try:
        cc = cache.botstats_data[bot.user.id]["commands_completed"]
    except KeyError:
        cc = 0
            ...
            try:
                try:
                    message = await bot.get_channel(
                        invoices[invoice]['message'][0]
                    ).fetch_message(invoices[invoice]['message'][1])
                except:
                    message = None
                if not invoices[invoice]['paid']:
                    if str(
                            p2p.check(str(invoices[invoice]
                                          ['bill_id'])).status) == "PAID":
                ...
                elif int(time.time()) > invoices[invoice][
                        'expires'] and not invoices[invoice]['paid']:
                    embed = discord.Embed()
